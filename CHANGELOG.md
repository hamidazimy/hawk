...
- The documented and CI-enforced compiler floor for Linux is corrected to GCC 13+ (previously misdocumented as GCC 12). Clang works at effectively any modern version when paired with libstdc++ 13+; paired with libc++ it specifically needs libc++ 17+, since libc++ gates `std::format` as experimental before then. A new configure-time CMake guard probes for the four C++20 library capabilities Hawk actually depends on (`std::format`, integer `from_chars`, calendar-validating `year_month_day`, `fast_float::from_chars`) by compiling real code rather than trusting feature-test macros, and fails fast with one message naming every gap instead of a cryptic error deep in some translation unit.
- macOS is now built and tested in CI (Apple Clang on `macos-latest`), but remains a tested bonus platform rather than a guaranteed one: no specific compiler version is promised the way Linux (GCC 13+/Clang 17+) and Windows (MSVC 2022) are, and `HAWK_STATIC_BUILD` is a documented no-op there, since Apple's toolchain has no static libc++/libSystem to link against.
- Windows build paths reorganized: the existing MinGW cross-compile script is renamed `build-mingw.sh` (previously `build-win.sh` — no longer unambiguous now that native Windows scripts exist too), and two native Windows scripts are added: `build-msvc.ps1` (MSVC 2022, static CRT) and `build-mingw.ps1` (native MinGW-w64, e.g. via MSYS2). All four build scripts now state `HAWK_BUILD_TESTS=OFF` explicitly rather than relying on CMake's default.
- `build.sh` no longer installs the binary to `~/.local` as a side effect; the built binary remains available at `build/bin/hawk` as documented.

### Fixed

- `parse_datetime`'s backend is replaced with a hand-rolled tokenizer and calendar validator, dropping its dependency on `std::chrono::parse`/`std::istringstream` — one of the least uniformly implemented corners of the C++20 standard library. This closes two real defects: MSVC's `std::chrono::parse` did not cross-check a parsed day against its month/year, so impossible dates like `2023-02-29` or `2024-04-31` parsed successfully and silently on Windows (the known gap noted in the 0.7.0 entry — the previously-excluded calendar-validation test now runs and passes on MSVC in CI); and Clang paired with libc++ could not build Hawk at all, since libc++ has never implemented `chrono::parse`. As a side effect of the rewrite, a UTC offset with no sign character (e.g. `0500`) is now rejected rather than silently treated as positive.
- Phase 2 of type inference (the full-file scan) now validates every DateTime candidate row with a real `parse_datetime` call instead of a cheap structural pre-screen that only checked digit positions and separators. A value like `2024-99-99 99:99:99` has the right shape but isn't a real instant; previously it passed the pre-screen and the column stayed confidently typed DateTime, with the bad row surfacing only later and indirectly as a skipped-row count during an unrelated filter or sort. The column's type is never downgraded by this — a count of invalid rows is tracked instead, since phase 1 already established the DateTime hypothesis from a real sample. Full validation is only affordable because of the faster parser above; the previous istringstream-based parser was roughly an order of magnitude slower.
- Floating-point `std::from_chars` is replaced with the vendored `fast_float` library at all three call sites that parse doubles. libc++ deletes the floating-point `from_chars` overload until a very recent LLVM version, which previously broke any Clang+libc++ build outright.
- The MinGW cross-compile target is now actually statically linked. `HAWK_STATIC_BUILD=ON` was a silent no-op there — CMake's `UNIX`/`WIN32` variables reflect the cross-compile target, not the host, so neither existing static-linking branch fired — leaving `hawk.exe`/`hawk-tests.exe` dynamically importing `libgcc_s_seh-1.dll` and `libstdc++-6.dll`, neither present on a clean Windows machine without a MinGW/MSYS2 install. A dedicated branch now applies the same static flags used on Linux, and a CI job cross-compiles and actually executes the resulting binary under Wine rather than only checking that it links.
- The console layer failed to compile on macOS: its POSIX includes (`<unistd.h>`, `<sys/ioctl.h>`) were gated behind `#ifdef __unix__`, which doesn't fire there, leaving `isatty`, `fileno`, and `ioctl` undeclared. The guard is now `#ifdef _WIN32` / `#else`, so every non-Windows platform — including macOS — gets the POSIX includes.
- Alongside that fix, `console.cpp` gained a guard against zero-width terminal reports from `ioctl()` (falling back to a named default width instead), Windows console setup now configures UTF-8 independently of virtual-terminal-processing support, and `terminal_width()` is stream-aware (defaulting to stdout) and returns `std::size_t` instead of `int`.
- Line endings are normalized via a new `.gitattributes`. Without it, git deferred to the checkout client's `core.autocrlf` setting, so a Windows checkout of an LF-committed test fixture converted it to CRLF and broke CRLF-detection's test assertion in CI — the code was correctly reporting what it was handed.
- Several documentation gaps a new contributor would hit immediately, found via a fresh-clone audit: a plain (non-recursive) clone left the replxx submodule uninitialized, failing the documented build at configure time; build instructions referenced sample files that don't exist or were never actually committed (`sample_logs/auth_small.csv` is now tracked for real); the architecture doc's `hawk-cli` tree diagram and a stale "tests are scaffolded but not yet populated" claim were both corrected; and a vendoring-rationale pointer to a file that isn't tracked in git — unresolvable from a fresh clone — was removed.

### Added

- GitHub Actions CI, building and running the full test suite on every push and pull request against master, across Linux (GCC), Windows (both native MSVC and a MinGW cross-compile executed under Wine), and macOS (Apple Clang) — plus a status badge in the README reflecting it.
- A manually-triggered release workflow that builds and publishes tagged GitHub releases with prebuilt Linux and Windows binaries, gated on the tagged commit's version matching both `CMakeLists.txt` and a corresponding `CHANGELOG.md` entry. Tagging and releasing remain two separate, individually-gated steps.
- The startup schema summary now surfaces phase-2 datetime validation results: one warning per DateTime column with a nonzero count of rows that failed real `parse_datetime` validation, naming the column, the count, and the pattern the rows failed to match. This required actually calling `confirm_schema` from `main` for the first time — the function existed already but was dead code, never invoked. As a result, every session's startup output now includes an "Inferred column types:" block, not just sessions with a mismatch; that block was previously absent entirely.

### Notes

- The MSVC datetime divergence noted as a known gap in the 0.7.0 entry is resolved by the parser rewrite above.
- Release binaries are deliberately Linux and Windows only. macOS is excluded from the release workflow: shipping an unsigned, unnotarized binary would fail Gatekeeper on first launch for most users, and Apple Developer Program enrollment for notarization is a cost this project hasn't taken on. macOS remains fully supported for building from source, and that build is verified in CI.
